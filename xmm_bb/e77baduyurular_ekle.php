...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form_kaydet")) {
  $insertSQL = sprintf("INSERT INTO duyurular_en (duyuru_adi, duyuru_metni, duyuru_link, duyuru_tarihi, duyuru_ekleyen_kullanici) VALUES (%s, %s, %s, %s, %s)",
  ...
  $insertGoTo = "e77baduyurular_ekle2.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_connection_lcl, $connection_lcl);
$query_rc_ekleyen_kullanici = "SELECT uye_user_adi FROM uyeler";
$rc_ekleyen_kullanici = mysql_query($query_rc_ekleyen_kullanici, $connection_lcl) or die(mysql_error());
$row_rc_ekleyen_kullanici = mysql_fetch_assoc($rc_ekleyen_kullanici);
$totalRows_rc_ekleyen_kullanici = mysql_num_rows($rc_ekleyen_kullanici);
...
do {  
?>
                       <option value="<?php echo strip_tags($row_rc_ekleyen_kullanici['uye_user_adi'])?>"<?php if (!(strcmp($row_rc_ekleyen_kullanici['uye_user_adi'], strip_tags($row_rc_ekleyen_kullanici['uye_user_adi'])))) {echo "selected=\"selected\"";} ?>><?php echo strip_tags($row_rc_ekleyen_kullanici['uye_user_adi'])?></option>
                       <?php
} while ($row_rc_ekleyen_kullanici = mysql_fetch_assoc($rc_ekleyen_kullanici));
  $rows = mysql_num_rows($rc_ekleyen_kullanici);
  if($rows > 0) {
      mysql_data_seek($rc_ekleyen_kullanici, 0);
	  $row_rc_ekleyen_kullanici = mysql_fetch_assoc($rc_ekleyen_kullanici);
  }
?>
                     </select>
...
<?php
mysql_free_result($rc_ekleyen_kullanici);
