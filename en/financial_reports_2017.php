...
$sayfa_adi="Financial Reports 2017";
...
$maxRows_rc_finansalrapor_en = 10;
$pageNum_rc_finansalrapor_en = 0;
if (isset($_GET['pageNum_rc_finansalrapor_en'])) {
  $pageNum_rc_finansalrapor_en = strip_tags($_GET['pageNum_rc_finansalrapor_en']);
}
$startRow_rc_finansalrapor_en = $pageNum_rc_finansalrapor_en * $maxRows_rc_finansalrapor_en;

mysql_select_db($database_connection_lcl, $connection_lcl);
$query_rc_finansalrapor_en = "SELECT * FROM finansal_raporlar_en where yil='2017'";
$query_limit_rc_finansalrapor_en = sprintf("%s LIMIT %d, %d", $query_rc_finansalrapor_en, $startRow_rc_finansalrapor_en, $maxRows_rc_finansalrapor_en);
$rc_finansalrapor_en = mysql_query($query_limit_rc_finansalrapor_en, $connection_lcl) or die(mysql_error());
$row_rc_finansalrapor_en = mysql_fetch_assoc($rc_finansalrapor_en);

if (isset($_GET['totalRows_rc_finansalrapor_en'])) {
  $totalRows_rc_finansalrapor_en = strip_tags($_GET['totalRows_rc_finansalrapor_en']);
} else {
  $all_rc_finansalrapor_en = mysql_query($query_rc_finansalrapor_en);
  $totalRows_rc_finansalrapor_en = mysql_num_rows($all_rc_finansalrapor_en);
}
$totalPages_rc_finansalrapor_en = ceil($totalRows_rc_finansalrapor_en/$maxRows_rc_finansalrapor_en)-1;
?>
...
<header class="template_baslik"><b>FINANCIAL REPORTS - 2017 YEARS</b></header>
<table width="670" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#F2F2F2" class="table_border">
  <tr>
    ...
    <table width="680" cellpadding="2" cellspacing="1" border="1" bordercolor="#CCCCCC">
      ...
      <?php do { ?>
        <tr>
          <td align="center" valign="middle" bgcolor="#f0f3f7" class="financialr_alt_baslik"><?php echo strip_tags($row_rc_finansalrapor_en['yil']); ?></td>
          <td align="left" valign="middle" bgcolor="#f0f3f7" class="financialr_alt_baslik"><?php echo strip_tags($row_rc_finansalrapor_en['aciklama']); ?> </td>
          <td align="center" valign="middle" bgcolor="#f0f3f7" class="financialr_alt_baslik"><a href="../upload/<?php echo strip_tags($row_rc_finansalrapor_en['link']); ?> " target="popup"><img src="../image/pdf_icon.png" width="16" height="16" border="0"/></a></td>
        </tr>
        <?php } while ($row_rc_finansalrapor_en = mysql_fetch_assoc($rc_finansalrapor_en)); ?>
    </table></td>
  </tr>
</table></td>        
      </tr>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rc_finansalrapor_en);
?>
<?
